...
(defun my/org-gtd-files ()
  "List my nessesary org gtd files"
  (seq-filter #'file-regular-p
              (append
               (list (concat org-directory "Организация/Входящее.org")
                     (concat org-directory "Организация/Рекомендации.org"))
               ...
               (file-expand-wildcards (concat org-directory "Контакты/*.org")))))

(defun my/prompt-my-org-gtd-files ()
  "Prompts for my org gtd files returned by my/org-gtd-files"
  (completing-read "GTD file: " (my/org-gtd-files)))
...
(after! org
  (setq org-capture-templates
        ...
        `(("g" "GTD")
          ...
          ("gi" "Quick task" entry  (file "Организация/Входящее.org")
           "%[~/.dotfiles/.config/doom/templates/entry-quick-task.org]"
           :prepend t :empty-lines-before 1 :clock-keep t :kill-buffer t)
          ;; TODO :refile-targets my/org-gtd-files - нужен список (FILES . SPECIFICATION)
          ("gt" "Full task" entry (file "Организация/Входящее.org")
           "%[~/.dotfiles/.config/doom/templates/entry-full-task.org]"
           :prepend t :empty-lines-before 1 :clock-keep t :kill-buffer t)
          ("ge" "Event" entry  (file "Организация/Входящее.org")
           "%[~/.dotfiles/.config/doom/templates/entry-event.org]"
           :prepend t :empty-lines-before 1 :clock-keep t :kill-buffer t)
          ("gh" "Habbit" entry (file "Организация/Входящее.org")
           "%[~/.dotfiles/.config/doom/templates/entry-habbit.org]"
           :prepend t :empty-lines-before 1 :clock-keep t :kill-buffer t)

          ("gT" "Task in project" entry (file+headline my/prompt-my-org-gtd-files "Задачи")
           "%[~/.dotfiles/.config/doom/templates/entry-full-task.org]"
           :prepend t :jump-to-captured t :unnarrowed t)
          ("gE" "Event in project" entry  (file+headline my/prompt-my-org-gtd-files "Расписание")
           "%[~/.dotfiles/.config/doom/templates/entry-event.org]"
           :prepend t :jump-to-captured t :unnarrowed t)
          ("gH" "Habbit in project" entry (file+headline my/prompt-my-org-gtd-files "Привычки")
           "%[~/.dotfiles/.config/doom/templates/entry-habbit.org]"
           :prepend t :jump-to-captured t :unnarrowed t)
...
#+begin_src emacs-lisp :tangle yes
(after! org
  ;; (setq doom-user-templates-dir (expand-file-name "templates/" doom-user-dir))

  (setq org-roam-capture-templates
        `(("c" "Записка, Черновик"
           plain (file "~/.dotfiles/.config/doom/templates/file-черновик.org")
           :target (file "Заметки/Входящие/%<%Y%m%d%H%M%S>.org")
           :unnarrowed t)
          ("f" "Фундаментальная, Мысль, Цитата"
           plain (file "~/.dotfiles/.config/doom/templates/file-фундаментальная_заметка.org")
           :target (file "Заметки/%<%Y%m%d%H%M%S>-${slug}.org")
           :unnarrowed t)
          ("n" "Заметка, Статья, Руководство"
           plain (file "~/.dotfiles/.config/doom/templates/file-заметка.org")
           :target (file "Заметки/%<%Y%m%d%H%M%S>-${slug}.org")
           :unnarrowed t)
          ("a" "Конспект"
           plain (file "~/.dotfiles/.config/doom/templates/file-конспект.org")
           :target (file "Заметки/конспект-${slug}.org")
           :unnarrowed t)
          ("r" "Рецепт"
           plain (file "~/.dotfiles/.config/doom/templates/file-рецепт.org")
           :target (file "Организация/Ценности/Питание/Рецепты/рецепт-${slug}.org")
           :unnarrowed t)
          ("x" "Копия"
           plain (file "~/.dotfiles/.config/doom/templates/file-копия.org")
           :target (file "Заметки/%<%Y%m%d%H%M%S>-${slug}.org")
           :unnarrowed t)
          ("p" "Контакт"
           plain (file "~/.dotfiles/.config/doom/templates/file-контакт.org")
           :target (file "Контакты/контакт-${slug}.org")
           :unnarrowed t)
          ("P" "Человек"
           plain (file "~/.dotfiles/.config/doom/templates/file-черновик.org")
           :target (file "Заметки/человек-${slug}.org")
           :unnarrowed t)

          ("o" "Организация")
          ("oa" "Ценность"
           plain (file "~/.dotfiles/.config/doom/templates/file-ценность.org")
           :target (file "Организация/Ценности/ценность-${slug}.org")
           :unnarrowed t)
          ("op" "Проект"
           plain (file "~/.dotfiles/.config/doom/templates/file-проект.org")
           :target (file "Организация/Проекты/${title}/проект-${slug}.org")
           :unnarrowed t)
          ("or" "План"
           plain (file "~/.dotfiles/.config/doom/templates/file-план.org")
           :target (file "Организация/Планирование/план-${slug}.org")
           :unnarrowed t))))
#+end_src

**** CANCELLED Org-roam-capture-templates tangle
CLOSED: [2026-04-21 Вт 04:25]
#+begin_src emacs-lisp :tangle no
...
#+begin_src emacs-lisp :tangle yes
(after! yasnippet
  ;; (setq yas-snippet-dirs '("~/.dotfiles/.config/doom/snippets")) ;; default (+snippets-dir doom-snippets-dir +file-templates-dir)

  (map! :map yas-keymap
        :after org
        "TAB" #'yas-next-field       ;; TODO сделать рабочим в org-mode
        "C-<tab>" #'yas-next-field)) ;; а не использовать этот костыль
#+end_src
** Org-tempo
